...
read -p "Do you want me to install DogFather Bot? (Y/N): "
...
# Install Luarocks
	...
	if [ ! -d .git ]; then
		echo -en "${Green}Would you like to clone the source of DogFather? (Y/N): ${Default}"
		read REPLY
		if [[ $REPLY == [yY] ]]; then
			echo -en "${Orange}Fetching latest DogFather source code\n${Default}"
			git clone -b master https://github.com/bhsgoclub/DogFather.git && cd DogFather
		fi
	fi
	...
	echo -en "${BGreen}DogFather successfully installed! Change values in config file and run ${BRed}./launch.sh${BGreen}.${Default}";;
	*) echo "Exiting...";;
esac
